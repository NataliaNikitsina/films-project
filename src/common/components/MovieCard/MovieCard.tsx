...
import {addFavoriteMovie, deleteFavoriteMovie, selectFavoriteMovies} from '@/app/model';
...
export const MovieCard = ({movie}: Props) => {
  ...
  const handleFavorites = (id: number) => {
    if (favoritesMovies.find((movie) => movie.id === id)) {
  ...
  const ratingColorStyle = movie.vote_average > 8 ? s.green : movie.vote_average > 5 ? s.yellow : s.red;

  const heartColorStyle = favoritesMovies.find((el) => el.id === movie.id) ? s.red : s.white;
  ...
  return (
    <article className={s.card}>
      <div className={s.poster}>
        ...
        <div className={s.favorite} onClick={() => handleFavorites(movie.id)}>
